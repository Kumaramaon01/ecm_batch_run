...
def main():
    ...
    output_csv = "database/Randomized_Sheet.xlsx"
    ...
    with col1:
        user_input = st.text_input("🌍 Enter the location name").strip().lower()
    #     output_inp_folder = st.text_input("Enter output folder path:", value="output_folder")
    with col2:
        output_inp_folder = st.text_input("Enter output folder path:", value="output_folder")
        inp_folder = output_inp_folder
    # with col4:
    #     run_cnt = st.number_input("Enter number", min_value=1, step=1, value=1, max_value=1500)

    # Available locations
    # st.write("##### 🌍 Available Locations")
    # st.write(weather_df['Sim_location'].str.replace('IND_', '').tolist())
    # user_input = st.text_input("Enter the location name from above list:").strip().lower()
    run_cnt = 1
    ...
    if not matched_row.empty:
        ...
        # st.success(f"Selected location: {matched_row.iloc[0]['Sim_location']}")
    elif user_input:
        st.error("❌ Location not found.")

    # Upload single INP file option
    ...
    if uploaded_file:
        ...
        st.success(f"Uploaded file saved to {uploaded_inp_path}")
    ...
    if st.button("🚀 Start Simulation"):
        ...
        for _, row in selected_rows.iterrows():
            ...
            # Apply modifications
            inp_content = wwr.process_window_insertion_workflow(inp_file_path, 1)
            inp_content = orient.updateOrientation(inp_content, 0)
            ...
            st.success(f"Saved modified INP: {new_inp_name}")
        ...
        if uploaded_file is None:
            ...
            st.markdown("##### 🔢 Starting Simulation")
            st.info(f"Output Simulation with {run_cnt} runs for {uploaded_file.name}")
        st.success("Simulation and data extraction complete!")
    ...
    with st.container():
        st.markdown(
            """
            <div style='display: flex; align-items: center; gap: 8px; font-size: 19px; justify-content: center;'>
