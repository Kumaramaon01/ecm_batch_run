...
def getCondUncondArea(inpfile):
    if isinstance(inpfile, list):
        if len(inpfile) == 0:
            print("Error: inpfile list is empty.")
            return None, None
        inpfile = inpfile[0]

    project_areas = {
        'narol': (12438, 48734, 61172, 51768.77),
        'gandhinagar': (19891, 63759.9, 44193.7, 39457.2),
        'churchroad': (62307.1, 148051.7, 143352, 67006.8),
        'icici': (40802.7, 11221.3, 52024, 0),
        'tcs': (460393.6, 564721, 651100, 374014.6),
        'crony': (253031.3, 118308.1, 347325.1, 24014.3),
        'surat': (16207.4, 51511.3, 47366.8, 20351.9),
        'eem': (112499.1, 112499.4, 224998.5, 0),
        'embassy': (178268.7, 71633.4, 226429.6, 23472.5),
        'mankind': (55062, 64600.1, 77622, 42040.1),
        'odhav': (37413, 100490.5, 68627.1, 69276.4),
        'dlf': (674177.9, 783231.4, 933661.7, 523748.8),
        'karle': (162418.1, 701518.1, 640704.8, 223231.4),
        'project 4': (823065, 2661288, 3470993, 13356.8)
    }

    with open(inpfile, 'r', encoding='utf-8', errors='ignore') as f:
        content = f.read().lower()

    for keyword, (cond_area, uncond_area, agarea, bgarea) in project_areas.items():
        if keyword in content:
            return cond_area, uncond_area, agarea, bgarea
...
def get_all_calculated_values(combined_data, inputPath):
    ...
    try:
        ...
        for file in gb.glob(os.path.join(inputPath, "*.sim")):
            ...
            lvbfiles = gb.glob(os.path.join(inputPath, f'{path}_lvb.csv'), recursive=True)
            lvdfiles = gb.glob(os.path.join(inputPath, f'{path}_lvd.csv'), recursive=True)
            ...
            FCTotal.append(areaCond)
            FUTotal.append(areaUncond)
            ...
            for lvbfile, lvdfile, zonefile, summary, lscfile, bepufile, svafile, psefile, shgcfile in zip(lvbfiles, lvdfiles, zonefiles, lvd_summary, lscfiles, bepufiles, svafiles, psefiles, shgcfiles):
                ...
                val1 = float(str(pse_data["TOTAL"].iloc[-2]).replace(',', '').strip())
                val2 = float(str(pse_data["TOTAL"].iloc[-1]).replace(',', '').strip())
                ratioPSE = val1 / val2
        ...
        combined_data['Power Lighting Total(W)'] = powerLight
        combined_data['Equipment-Total(W)'] = EquipTotal
...
def get_shgc_data(file):
    shgc_data = pd.read_csv(file)
