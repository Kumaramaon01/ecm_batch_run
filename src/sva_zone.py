...
def get_SVA_Zone_report(name, path):
    try:
        with open(name) as f:
            flist = f.readlines()
            ...
            sva_zone.columns = ['ZONE_NAME', 'SUPPLY-FLOW(CFM)', 'EXHAUST-FLOW(CFM)',
                            'FAN(KW)', 'MINIMUM(FLOW)(FRAC)', 'OUTISIDE-AIR-FLOW(CFM)',
                            'COOLING_CAPACITY(KBTU/HR)', 'SENSIBLE(FRAC)', 'EXTRACTION-RATE(KBTU/HR)', 'HEATING_CAPACITY(KBTU/HR)', 
                            'ADDITION-RATE(KBTU/HR)', 'ZONE-MULT']
        ...
        columns = ['RUNNAME', 'ZONE_NAME', 'SUPPLY-FLOW(CFM)', 'EXHAUST-FLOW(CFM)',
                            'FAN(KW)', 'MINIMUM(FLOW)(FRAC)', 'OUTISIDE-AIR-FLOW(CFM)',
                            'COOLING_CAPACITY(KBTU/HR)', 'SENSIBLE(FRAC)', 'EXTRACTION-RATE(KBTU/HR)', 'HEATING_CAPACITY(KBTU/HR)', 
                            'ADDITION-RATE(KBTU/HR)', 'ZONE-MULT']
